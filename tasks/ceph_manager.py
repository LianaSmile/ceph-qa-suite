...
DEFAULT_CONF_PATH = '/etc/ceph/xtao.conf'

log = logging.getLogger(__name__)


def write_conf(ctx, write_subsys, write_key, write_value=None, write_add=True, conf_path=DEFAULT_CONF_PATH):
    # Liyan debug
    pyscript = """
import ConfigParser
import string
import commands
def set_config(conf_path, conf_section, conf_key, conf_value, conf_add=True):
    config = ConfigParser.ConfigParser()
    config.read(conf_path)
    if conf_add:   # add conf
        ...
        if config.has_section(conf_section):
            if config.has_option(conf_section,conf_key):
                config.remove_option(conf_section,conf_key)
                config.remove_section(conf_section)
                config.write(open(conf_path,'w'))
    commands.getstatusoutput("xd mon putconf xtao")
    for section in config.sections():
        print '['+section+']'
        for option in config.options(section):
            print option,' = ',config.get(section,option)
set_config("{xtao_path}","{xtao_section}","{xtao_key}","{xtao_value}",conf_add={conf_add})
""".format(
             xtao_path=conf_path,
             xtao_section=write_subsys,
             xtao_key=write_key,
             xtao_value=write_value,
             cluster_name=ctx.config['cluster'],
             conf_add=write_add)
    ...
    #teuthology.feed_many_stdins_and_close(pyscript, writes)
    run.wait(writes)
    log.info('Liyan write xtao.conf print {out}'.format(out=writes))
...
class CephManager:
    ...
    def raw_cluster_cmd_xtcli(self,*args):
        """
        Start xtcli to run ... 
        """
        testdir = teuthology.get_testdir(self.ctx)
