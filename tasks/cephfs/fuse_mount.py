...
class FuseMount(CephFSMount):
    def __init__(self, client_config, test_dir, client_id, client_remote, cluster_name='xtao'):
        super(FuseMount, self).__init__(test_dir, client_id, client_remote, cluster_name='xtao')
    ...
    def mount(self, mount_path=None, mount_fs_name=None):
       ...
       # Liyan add if
        ...
        # Liyan add create auth
        """
        client_keyring = \
                        '/etc/ceph/ceph.client.{id}.keyring'.format(id=self.client_id)
        self.client_remote.run(
            args=[
                'cd',
        ...
        set_conf('client.{id}'.format(id=self.client_id),'keyring',client_keyring) 
        """
        ...
        def list_connections():
            # Liyan modified
            f = self.client_remote.run(
                args=["ls","/sys/fs",run.Raw('|'),"grep","fuse"],
                stdout=StringIO(),
                check_status=False
            )
            if f.exitstatus == 0:
                ...
                log.info('testing node does not exist the direction of /sys/fs/fuse/')
                return []
    ...
    def umount(self):
        ...
        if m.exitstatus == 0:
            ...
            #Peng temp allow /mnt/cephfuse mount point exists
    ...
    def teardown(self):
        ...
        if self.is_mounted():
            self.client_remote.run(
                args=[
                    'rm',
                    '-rf',
                    '{mountpoint}/*'.format(mountpoint=self.mountpoint),
                ],
            )
        self.umount()
        ...
        pyscript = """
import os
os.path.exists('{mountpoint}')
""".format(mountpoint=self.mountpoint)
        
        os_p = self.client_remote.run(
            args=[
                'python',
                 '-c',
                 pyscript
        ...
        if os_p.exitstatus == 0:
            ...
            log.info("{mountpoint} was already tearndown,do not need tearndown again".format(mountpoint=self.mountpoint))

    def _asok_path(self):
        # Liyan debug
        #return "/var/run/ceph/ceph-client.{0}.*.asok".format(self.client_id)
        return "/var/run/ceph/xtao-client.{0}.asok".format(self.client_id)
...
""".format(
            ...
            client_name="xtao-client.{0}".format(self.client_id))
