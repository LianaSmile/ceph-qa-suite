...
class CephCluster(object):
    ...
    def __init__(self, ctx):
        self._ctx = ctx
        log.info('Liyan debug cluster name is {cluster_name}'.format(cluster_name=self._ctx.config['cluster']))
        self.mon_manager = ceph_manager.CephManager(self.admin_remote, ctx=ctx, logger=log.getChild('ceph_manager'),cluster=self._ctx.config['cluster'])
    ...
    def set_ceph_conf(self, subsys, key, value):
        # Liyan debug
        """if self._ctx.config['ceph'].get('conf') == None:
            self._ctx.config['ceph']['conf'] = {}        

        if subsys not in self._ctx.config['ceph'].get('conf'):
            self._ctx.config['ceph']['conf'][subsys] = {}
        self._ctx.config['ceph']['conf'][subsys][key] = value"""
        #write_conf(self._ctx,cluster=self._ctx.config['cluster'])
        write_conf(self._ctx,subsys,key,write_value=value)
    ...
    def clear_ceph_conf(self, subsys, key):
        ...
        write_conf(self._ctx, subsys, key, write_add=False)
...
class MDSCluster(CephCluster):
    ...
    def __init__(self, ctx):
        ...
        log.info('Liyan debug cluster name is {cluster_name}'.format(cluster_name=ctx.config['cluster']))
        self.mds_ids = list(misc.all_roles_of_type(ctx.cluster, 'mds'))
        # Liyan debug
        clustername = ctx.config['cluster']
        ...
        if hasattr(self._ctx, "daemons"):
            ...
            self.mds_daemons = dict([(mds_id, self._ctx.daemons.get_daemon('mds', mds_id,clustername)) for mds_id in self.mds_ids])
    ...
    def mds_stop(self, mds_id=None):
        ...
        """
        for mds_id in self.mds_ids:
            cluster, typename = self.mds_daemons[mds_id].role.split('.')
            nodeid = self.mds_daemons[mds_id].id_
            run_cmd = ['docker',
                       'stop',
                       '%s-%s.%s' % (cluster, typename, nodeid)
                ]

        self._one_or_all(mds_id, lambda id_: self.mds_daemons[id_].stop_with_args(run_cmd))
...
class Filesystem(MDSCluster):
    ...
    def add_data_pool(self, name):
        ...
        self.mon_manager.raw_cluster_cmd_xtcli('pool','create',new_pool_name,'-o','hdd','-n','2')
    ...
    def put_metadata_object_raw(self, object_id, infile):
        ...
        #Liyan modified
        cluster,_,_ = misc.get_first_mon(self._ctx,None).split('.')

        self.client_remote.run(args=[
            'sudo', os.path.join(self._prefix, 'rados'), '--cluster',cluster  ,'-p', self.metadata_pool_name, 'put', object_id, temp_bin_path
        ])
    ...
    def get_metadata_object_raw(self, object_id):
        ...
        self.client_remote.run(args=[
            'sudo', os.path.join(self._prefix, 'rados'),'--cluster',cluster, '-p', self.metadata_pool_name, 'get', object_id, temp_bin_path
        ])
    ...
    def _read_data_xattr(self, ino_no, xattr_name, type, pool):
        ...
        args = [
            os.path.join(self._prefix, "rados"), "-p", pool, "getxattr", obj_name, xattr_name, '--cluster',cluster
        ]
        try:
            proc = remote.run(
                args=args,
    ...
    def _write_data_xattr(self, ino_no, xattr_name, data, pool=None):
        ...
        cluster,_,_ = misc.get_first_mon(self._ctx.None).split('.')
        args = [
            os.path.join(self._prefix, "rados"), "--cluster", cluster, "-p", pool, "setxattr",
    ...
    def rados(self, args, pool=None, namespace=None, stdin_data=None):
        ...
        # Liyan modified
        cluster,_,_ = misc.get_first_mon(self._ctx.None).split('.')
        args = ([os.path.join(self._prefix, "rados"),"--cluster",cluster, "-p", pool] +
